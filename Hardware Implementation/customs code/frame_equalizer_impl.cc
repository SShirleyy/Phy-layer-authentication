...
frame_equalizer_impl::frame_equalizer_impl(Equalizer algo, double freq, double bw, bool log, bool debug) :
	...
	message_port_register_out(pmt::mp("channel"));
	...
	syncfile.open("/tmp/sync");
	chanfile.open("/tmp/channel");

	for (int n =0;n < 52;n++) {
		chan_avg[n] = gr_complex(0,0);
	}
}
...
frame_equalizer_impl::general_work (int noutput_items,
	...
	while((i < ninput_items[0]) && (o < noutput_items)) {
		...
		d_equalizer->equalize(current_symbol, d_current_symbol,
				...
				pilots[0] = current_symbol[11];
				pilots[1] = current_symbol[25];
				pilots[2] = current_symbol[39];
				pilots[3] = current_symbol[53];
		/*
		int ofs = 6;
		syncfile << d_frame_symbols << " " << d_current_symbol << "\n";
		bool zero = false;
		for (int n = 0; n < sizeof(current_symbol)/sizeof(gr_complex); n++) {

			int pos2 = n-6;
			if((n == 11) || (n == 25) || (n == 39) || (n == 53) || (n < 6) || ( n >= 58)) {
				if((n > 6) && ( n < 58)) {
					ofs++;
					chan_avg[pos2] = current_symbol[n] * p;
					if (n == 53) {
						chan_avg[pos2] = -chan_avg[pos2];
					}
					if((chan_avg[pos2].real() != 0) || (chan_avg[pos2].imag() != 0)){
						//chanfile << chan_avg[pos2] << " ";
					} else {
						zero = true;
						continue;
					}
					//mylog(boost::format("current_symbol[%1%]: %2% - chan_avg[%3%]: %4%")
					//		% n % current_symbol[n] % pos2 % chan_avg[pos2]);
				}
			}
			else {
				int pos = n-ofs;
				if((symbols[pos].real() != 0) || (symbols[pos].imag() != 0)){
					chan_avg[pos2] = current_symbol[n] / symbols[pos];
					//chanfile << chan_avg[pos2] << " ";
				} else {
					zero = true;
		...
		if (d_current_symbol == 1) {
			gr_complex *d_H = d_equalizer->get_d_H();
			message_port_pub(pmt::mp("channel"), pmt::cons(pmt::make_dict(), pmt::init_c32vector(64, d_H)));
		}
		// signal field
		if(d_current_symbol == 2) {
		...
		if(d_current_symbol > 2) {
			...
			//mylog(boost::format("pilot1: %1% - pilot2: %2% - pilot3: %3% - pilot4: %4%")
			//		% d_prev_pilots[0] % d_prev_pilots[1] % d_prev_pilots[2] % d_prev_pilots[3]);
			//realfile << d_prev_pilots[0].real() << " " << d_prev_pilots[1].real() << " " << d_prev_pilots[2].real() << " " << d_prev_pilots[3].real() << "\n";
			//imgfile << d_prev_pilots[0].imag() << " " << d_prev_pilots[1].imag() << " " << d_prev_pilots[2].imag() << " " << d_prev_pilots[3].imag() << "\n";
	...
	/*
	if(pmt::dict_has_key(dict,pmt::string_to_symbol("snr"))){
		myfile << "dict: " << pmt::dict_items(dict) << " | symbols: " << pmt::init_c32vector(48, symbols) << "\n\n";
	}
	*/
